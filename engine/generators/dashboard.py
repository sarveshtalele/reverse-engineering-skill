...
- Dependency graph renders with vis-network (interactive, physics-based).
...
/* ================================================================
   ...
   ================================================================ */
/* Wrapper provides the positioning context AND clips canvas to
   rounded corners via overflow:hidden. The legend is inside the
   wrapper so its position:absolute anchors correctly. */
.network-wrap {{
  position: relative;
  border-radius: var(--radius);
  overflow: hidden;
...
/* The actual vis.js canvas target — fills the wrapper entirely */
.network-card {{
  width: 100%;
  height: 100%;
}}
    ...
    <div class="content-area">
      ...
      <div class="card" style="margin-bottom:20px">
        ...
        <!-- vis-network interactive dependency graph.
             ...
             positioning anchor for .network-legend (position:absolute). -->
        <div class="network-wrap">
          <div class="network-card" id="arch-network"></div>
          <div class="network-legend">
            <span><span class="legend-dot" style="background:#0071e3"></span>Controller</span>
            <span><span class="legend-dot" style="background:#30d158"></span>Service</span>
            <span><span class="legend-dot" style="background:#ff9f0a"></span>Repository</span>
...
function switchSection(name, btn) {{
  ...
  if (name === 'architecture') initNetwork();
...
// ----------------------------------------------------------------
// vis.js Network — interactive dependency graph
// ----------------------------------------------------------------
...
function initNetwork() {{
  ...
  // Set flag inside RAF so a CDN/render failure doesn't permanently block
  // the graph from being re-attempted on the next Architecture tab click.
  requestAnimationFrame(() => {{
    try {{
        ...
        {{
          physics: {{
            ...
            forceAtlas2Based: {{ gravitationalConstant:-40, centralGravity:0.005,
                                 springLength:100, springConstant:0.08, damping:0.4 }},
            stabilization: {{ iterations: 150, updateInterval: 25 }},
          }},
          layout:      {{ improvedLayout: false }},
          interaction: {{ zoomView: true, dragView: true, hover: true,
                          tooltipDelay: 100, navigationButtons: false }},
          edges:       {{ smooth: {{ type:'dynamic' }} }},
          nodes:       {{ shadow: {{ enabled:true, size:4, x:2, y:2, color:'rgba(0,0,0,0.08)' }} }},
        }}
      );
      network.once('stabilizationIterationsDone', () => {{
        network.setOptions({{ physics: false }});
      }});
      networkInited = true;   // only lock after successful creation
    }} catch (err) {{
      console.error('vis.Network failed:', err);
      container.innerHTML = '<p style="padding:40px 24px;font-size:13px;color:var(--text2)">Graph render failed — check browser console.</p>';
    }}
  }});
}}
